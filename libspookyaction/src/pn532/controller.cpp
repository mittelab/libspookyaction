...
namespace pn532 {
    ...
    const std::vector<target_type> controller::poll_all_targets = {
    ...
    namespace {
        template <class... Args>
        result<bool> nfc_diagnose_simple(
    ...
    result<> controller::write_gpio(gpio_status const &status, bool write_p3, bool write_p7, ms timeout) {
        ...
        if (write_p3) {
            payload << std::uint8_t(bits::gpio_write_validate_max | status.mask(gpio_port::p3));
        } else {
            payload << std::uint8_t{0x00};
        }
        if (write_p7) {
            payload << std::uint8_t(bits::gpio_write_validate_max | status.mask(gpio_port::p7));
        } else {
            payload << std::uint8_t{0x00};
        }
    ...
    result<> controller::set_gpio_pin(gpio_port loc, std::uint8_t pin_idx, bool value, ms timeout) {
        ...
        if (auto res_read = read_gpio(rt.remaining()); res_read) {
            ...
            const bool write_p7 = (loc == gpio_port::p7);
    ...
    result<std::vector<target_kbps106_typea>> controller::initiator_list_passive_kbps106_typea(
    ...
    result<std::vector<target_kbps106_typea>> controller::initiator_list_passive_kbps106_typea(
            nfcid_1t uid, std::uint8_t max_targets, ms timeout) {
        sanitize_max_targets(max_targets, "initiator_list_passive_kbps106_typea");
        return initiator_list_passive<baudrate_modulation::kbps106_iso_iec_14443_typea>(
    ...
    result<std::vector<target_kbps106_typea>> controller::initiator_list_passive_kbps106_typea(
            nfcid_2t uid, std::uint8_t max_targets, ms timeout) {
        sanitize_max_targets(max_targets, "initiator_list_passive_kbps106_typea");
        return initiator_list_passive<baudrate_modulation::kbps106_iso_iec_14443_typea>(
                max_targets, bin_data::chain(uid), timeout);
    }
    ...
    result<std::vector<target_kbps106_typea>> controller::initiator_list_passive_kbps106_typea(
            nfcid_3t uid, std::uint8_t max_targets, ms timeout) {
        sanitize_max_targets(max_targets, "initiator_list_passive_kbps106_typea");
        return initiator_list_passive<baudrate_modulation::kbps106_iso_iec_14443_typea>(
                max_targets, bin_data::chain(uid), timeout);
    }

    result<std::vector<target_kbps106_typeb>> controller::initiator_list_passive_kbps106_typeb(
    ...
    result<std::vector<target_kbps212_felica>> controller::initiator_list_passive_kbps212_felica(
        ...
        return initiator_list_passive<baudrate_modulation::kbps212_felica>(
                max_targets, bin_data::chain(payload), timeout);
    }

    result<std::vector<target_kbps424_felica>> controller::initiator_list_passive_kbps424_felica(
            std::array<std::uint8_t, 5> const &payload, std::uint8_t max_targets, ms timeout) {
        sanitize_max_targets(max_targets, "initiator_list_passive_kbps424_felica");
        return initiator_list_passive<baudrate_modulation::kbps424_felica>(
                max_targets, bin_data::chain(payload), timeout);
    }

    result<std::vector<target_kbps106_jewel_tag>> controller::initiator_list_passive_kbps106_jewel_tag(ms timeout) {
    ...
    result<rf_status, bin_data> controller::initiator_data_exchange(
        ...
        for (std::size_t chunk_idx = 0; chunk_idx < n_chunks; ++chunk_idx) {
            ...
            if (res_cmd->first.error != internal_error_code::none) {
                if (more_data) {
