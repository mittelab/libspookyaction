...
namespace ut::desfire_ciphers {
    ...
    void test_des() {
        {
            ...
            auto c = always_default_cipher_provider{}.cipher_from_key(key<cipher_type::des>{});
            {
                {
    ...
    void test_aes_kdf() {
        const std::array<std::uint8_t, 16> k{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff};
        bin_data div_data = {0x04, 0x78, 0x2E, 0x21, 0x80, 0x1D, 0x80, 0x30, 0x42, 0xF5, 0x4E, 0x58, 0x50, 0x20, 0x41, 0x62, 0x75};
        const auto div_key = desfire::esp32::default_cipher_provider{}.diversify_key_an10922(key<cipher_type::aes128>{0, k}, div_data);
        ...
        TEST_ASSERT_EQUAL_HEX8_ARRAY(std::begin(div_key.get<cipher_type::aes128>().k), std::begin(exp_div_key), 16);
    }

    void test_3k3des_kdf() {
        const std::array<std::uint8_t, 24> k{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08};
        bin_data div_data = {0x04, 0x78, 0x2E, 0x21, 0x80, 0x1D, 0x80, 0x30, 0x42, 0xF5, 0x4E, 0x58, 0x50};
        const auto div_key = desfire::esp32::default_cipher_provider{}.diversify_key_an10922(key<cipher_type::des3_3k>{0, k}, div_data);
        ...
        TEST_ASSERT_EQUAL_HEX8_ARRAY(std::begin(div_key.get<cipher_type::des3_3k>().k), std::begin(exp_div_key), 16);
    }

    void test_2k3des_kdf() {
        const std::array<std::uint8_t, 16> k{0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
        bin_data div_data = {0x04, 0x78, 0x2E, 0x21, 0x80, 0x1D, 0x80, 0x30, 0x42, 0xF5, 0x4E, 0x58, 0x50, 0x20, 0x41};
        const auto div_key = desfire::esp32::default_cipher_provider{}.diversify_key_an10922(key<cipher_type::des3_2k>{0, k}, div_data);

        const std::array<std::uint8_t, 24> exp_div_key{0x16, 0xF9, 0x58, 0x7D, 0x9E, 0x89, 0x10, 0xC9, 0x6B, 0x96, 0x48, 0xD0, 0x06, 0x10, 0x7D, 0xD7};

        TEST_ASSERT_EQUAL_HEX8_ARRAY(std::begin(div_key.get<cipher_type::des3_2k>().k), std::begin(exp_div_key), 16);
    }
