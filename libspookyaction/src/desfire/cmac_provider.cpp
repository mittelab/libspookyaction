...
namespace desfire {
    ...
    void cmac_provider_base::prepare_subkey(range<std::uint8_t *> subkey, std::uint8_t last_byte_xor) {
    ...
    void cmac_provider_base::initialize_subkeys() {
    ...
    void cmac_provider_base::do_cmac_crypto(mlab::bin_data &buffer, range<std::uint8_t *> iv, range<const std::uint8_t *> data) {
        ...
        buffer.clear();
        buffer.resize(padded_length(data.size(), block_size()));
        ...
        if (buffer.size() == data.size()) {
            // Was not padded
            std::transform(std::begin(last_block), std::end(last_block), key_nopad().begin(),
                           std::begin(last_block), xor_op);
        } else {
            // Was padded, but spec wants to pad with 80 00 .. 00, so change one byte
            buffer[data.size()] = 0x80;
            std::transform(std::begin(last_block), std::end(last_block), key_pad().begin(),
                           std::begin(last_block), xor_op);
        }

        // Return the first 8 bytes of the last block
        crypto_implementation().do_crypto(buffer.data_view(), iv, crypto_operation::mac);
    }
    ...
    cmac_provider::mac_t cmac_provider::compute_cmac(range<std::uint8_t *> iv, range<std::uint8_t const *> data) {
        ...
        // Return the first 8 bytes of the last block
        do_cmac_crypto(_cmac_buffer, iv, data);
